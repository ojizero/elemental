defmodule Elemental.Support.Toast do
  ...
  def alert_message(assigns) do
    ...
    <Alert.alert id={@id} type={@type} outline={@outline} dash={@dash} soft={@dash}>
      ...
      <.close_button target_id={@id} type={@type} />
    </Alert.alert>
    """
  end
  ...
  attr :type, :string, values: [nil, "info", "success", "warning", "error"], default: nil
  attr :clear_alert_event, :string, default: "el:clear-alert"
  ...
  defp close_button(assigns) do
    ...
    <Button.button
      ...
      shape="circle"
      size="xs"
      phx-click={close_alert(@target_id, @clear_alert_event)}
    >
      <.x_mark />
    </Button.button>
    """
  ...
  defp close_alert(js \\ %JS{}, id, alert_cleared_event) do
    selector = "##{Phoenix.HTML.css_escape(id)}"

    js
    |> JS.set_attribute({"hidden", true}, to: "#{selector}")
    # TODO: when should i call clear flash?
    # |> JS.push("lv:clear-flash", value: %{key: @type})
    # TODO: make this configurable
    |> JS.push(alert_cleared_event, value: %{id: id})
  end
